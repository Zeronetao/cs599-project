...
from .constant import analysis_num, gossip_num_max, with_photo, without_ana
import pandas as pd
...
def extract_for_choose_buy(choose_buy):
    if "hold" in choose_buy or "Hold" in choose_buy:
        ...
        try:
            match = re.search(
                r"^\s*Operation:\s*buy,\s*Stock name:\s*(?:Stock\s+)?\s*([A-Z]),\s*Investment Amount:\s*\$?(\d+(\.\d+)?),\s*Best Buying Price:\s*\$?(\d+(\.\d+)?)\s*$",
                choose_buy,
                re.IGNORECASE,
            )
            if match:
    ...
    if "hold" in choose_sell or "Hold" in choose_sell:
        ...
        try:
            match = re.search(
                ...
                #r"^\s*Operation:\s*(buy|sell),\s*Stock name:\s*([A-Z]),\s*The number of shares:\s*(\d+),\s*Best Price:\s*\$?(\d+(\.\d+)?)\s*$",
                choose_sell,
                re.IGNORECASE,
            )
            if match:
...
def stock_ops(virtual_date, persons, stocks, market_index, iter):
    ...
    for p in persons:
        if p.person_id > -1:
            if p.person_id in without_ana and virtual_date>=2:
                choose_buy = run_gpt_prompt_choose_buy_stock(
                    virtual_date, p, stocks,[]
                )
    ...
    for p in persons:
        if p.person_id > -1:
            if p.reflect_frequency == 0:
                pass
            elif (iter + 1) % p.reflect_frequency == 0:# and virtual_date % 2 == 0:
                ...
                #在这里调用strategy和reward给下一步策略更新做指导
