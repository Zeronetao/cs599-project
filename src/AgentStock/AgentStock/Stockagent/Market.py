    ...
    def _fetch_orders(self, type, fetch_stock_id):
        if type == "buy":
            fetch_cmd = (
                ...
                .format(type, fetch_stock_id)#"order by price ASC,timestamp ASC;"
            )
        elif type == "sell":
            fetch_cmd = (
                ...
                "select * from (select * from active_orders where type ='{type}' and stock_id={id} "
                "and status='active' and "
                "person_id>=0); ".format(type="sell", id=fetch_stock_id)  
            )#order by price DESC,timestamp ASC);order by price DESC,timestamp ASC)
        elif type == "all":
            fetch_cmd = (
