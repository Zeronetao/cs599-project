...
from .constant import expense_ratio, STOCK_NAMES, with_stock_infor
...
class Broker:
    ...
    def ipo(self, virtual_date):
        for each_stock in self.stocks:
            if self.inventories[each_stock.stock_id] > 0:#每只股票的份数
                submit_order(
                    self.db,
                    "sell",
                    self.person_id,
...
class Person:
    ...
    def create_order(self, i, op, virtual_date, iteration=0):
        ...
        order_type = order_type.lower()  # convert to lowercase小写

        # timestamp int, virtual_date text, weekday int, stock_id int, person_id int, type text, price float
        if order_type == "buy":#这里相当于做出交易
            cash_expense = bid_price * quantity#数量*价格
            bid_price = bid_price * 1.05  #数量*1.05？
    ...
    def query_prompt(self, virtual_date):
        ...
        for each_hold_stock in results:
            ...
            if self.person_id in with_stock_infor:
                dic = {
